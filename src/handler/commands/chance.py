...
class Chance(Base):
    ...
    def execute(self, command):
        ...
        try:
            ...
            old_chance = chance_repository.set(
                chat_id=command.chat_id, new_chance=new_chance
            )

            self.reply(
                command, "Change chance from {} to {}".format(old_chance, new_chance)
            )
        except (IndexError, ValueError):
            self.reply(
                command,
                "Current chance: {}".format(chance_repository.get(command.chat_id)),
            )
