...
class StatusHandler(MessageHandler):
    def __init__(self, chat_purge_queue):
        super(StatusHandler, self).__init__(Filters.status_update, self.handle)
    ...
    def __process_bot_kick(self, status):
        logging.debug("[Chat %s %s bot_kicked]" % (status.chat_type, status.chat_id))

        self.chat_purge_queue.add(status.chat_id)

    def __process_bot_add(self, status):
        logging.debug("[Chat %s %s bot_added]" % (status.chat_type, status.chat_id))
