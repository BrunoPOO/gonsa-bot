...
class MessageHandler(ParentHandler):
    def __init__(self):
        super(MessageHandler, self).__init__(
            Filters.text | Filters.sticker | Filters.photo, self.handle
        )
    ...
    def __check_media_uniqueness(self, bot, message):
        if (
            ...
            logging.debug(
                "[Chat %s %s not unique media]" % (message.chat_type, message.chat_id)
            )
    ...
    def __process_message(self, bot, message):
        logging.debug(
            "[Chat %s %s message length] %s"
            % (message.chat_type, message.chat_id, len(message.text))
        )
        ...
        if should_answer:
            ...
            if text is None:
                return
            reply_id = (
                None if not message.is_reply_to_bot() else message.message.message_id
            )

            logging.debug(
                "[Chat %s %s answer/reply] %s"
                % (message.chat_type, message.chat_id, text)
            )

            bot.send_message(
                chat_id=message.chat_id, reply_to_message_id=reply_id, text=text
            )

    def __process_sticker(self, bot, message):
        if message.has_anchors() or message.is_private() or message.is_reply_to_bot():
            logging.debug(
                "[Chat %s %s spam_sticker]" % (message.chat_type, message.chat_id)
            )
