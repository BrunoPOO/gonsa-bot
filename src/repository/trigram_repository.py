...
class TrigramRepository(RedisRepository):
    ...
    def find_word(self, chat_id, similar_word, max_results=10):
        ...
        for pair in redis.scan_iter(match=search_pattern, count=max_results):
            (first, second) = (
                pair.decode(encoding).lstrip(format_pattern).split(self.separator)
            )
    ...
    def remove_word(self, chat_id, exact_word):
        ...
        """
        self.__remove_keys(
            self.source_name.format(chat_id, exact_word + "\\" + self.separator + "*")
        )
        self.__remove_keys(
            self.source_name.format(chat_id, "*" + "\\" + self.separator + exact_word)
        )
